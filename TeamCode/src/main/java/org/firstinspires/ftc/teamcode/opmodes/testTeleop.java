...
public class testTeleop extends ActionOpMode {
    ...
    public void loop() {
        ...
        if (gamepad2.x && !gamepad2XPressed) {
            ...
            if (motorControl.lift.motor.getCurrentPosition() > 100)
                run(motorActions.lift.secondBucket());
        } else if (!gamepad2.x) {
...
// LEFT BUMPER
        ...
        if (gamepad1.dpad_down && !dpadDownPressed) {
            ...
            if (down == DetectedColor.UNKNOWN || down == DetectedColor.BLACK) {
                extendPosition = 500; // If nothing in intake, extend to 500
            } else if (down == allianceColor || down == DetectedColor.YELLOW) {
                extendPosition = 500; // Correct color → Extend to 500
            } else {
