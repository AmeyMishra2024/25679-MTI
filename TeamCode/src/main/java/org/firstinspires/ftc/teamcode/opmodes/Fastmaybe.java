...
import org.firstinspires.ftc.teamcode.helpers.hardware.actions.Fastpathchain;
 ...
 */
@Autonomous(name = "Fastboom")
public class Fastmaybe extends Fastpathchain {
    ...
    private final Pose scorePose1 = new Pose(41, 70, Math.toRadians(0));
    private final Pose scorePose2 = new Pose(41, 69, Math.toRadians(0));
    private final Pose scorePose3 = new Pose(41, 68, Math.toRadians(0));
    private final Pose scorePose4 = new Pose(41, 67 , Math.toRadians(0));
    ...
    private final Pose plus2pose = new Pose(11, 74, Math.toRadians(270));

    private final Pose parkControlPose = new Pose(12, 74, Math.toRadians(300));
    private final Pose sampleScorePose   = new Pose(12, 120, Math.toRadians(280));
    ...
    private PathChain pickupsample, pickupsample2;
    private PathChain finalparking;
    private PathChain ScoreBasket, basket2;
    ...
    protected void buildPathChains() {
        ...
        ScoreBasket = follower.pathBuilder()
                ...
                .addParametricCallback(0.2, () -> run(motorActions.outtakeSample()))
        ...
        pickupsample2 = follower.pathBuilder()
                ...
                .addParametricCallback(0.3, () -> run(motorActions.extendo.setTargetPosition(500)))
                .build();

        basket2 = follower.pathBuilder()
                .addPath(new BezierCurve(
                        new Point(plus2pose),      // Start from the park position
                        //new Point(parkControlPose), // Intermediate control point (optional)
                        new Point(sampleScorePose)))     // End at scorePose
                .setLinearHeadingInterpolation(parkPose.getHeading(), sampleScorePose.getHeading())
                .setZeroPowerAccelerationMultiplier(4) // Adjust acceleration to control speed
    ...
    protected void buildTaskList() {
        ...
        // Preload task.
        PathChainTask preloadTask = new PathChainTask(scorePreload, 0.01);
        tasks.add(preloadTask);

        PathChainTask pickUpTask1 = new PathChainTask(grabPickup1, 0.01);
        tasks.add(pickUpTask1);

// Deposit task 1.
        PathChainTask depositTask1 = new PathChainTask(depositHP1, 0.01);
        tasks.add(depositTask1);



// Pickup task 2.
        PathChainTask pickUpTask2 = new PathChainTask(grabPickup2, 0.01);
        tasks.add(pickUpTask2);

// Deposit task 2.
        PathChainTask depositTask2 = new PathChainTask(depositHP2, 0.01);
        tasks.add(depositTask2);

// Pickup task 3.
        PathChainTask pickUpTask3 = new PathChainTask(grabPickup3, 0.01);
        tasks.add(pickUpTask3);

// Deposit task 3.
        PathChainTask depositTask3 = new PathChainTask(depositHP3, 0.01);
        tasks.add(depositTask3);

// Intake task 1.
        PathChainTask intakeTask1 = new PathChainTask(intake1, 0.01);
...
// Park task.
        tasks.add(new PathChainTask(pickupsample, 0));

// New task: Move from park to scorePose.
// If you need to run additional actions (previously triggered by wait actions) you can execute them immediately after the path completes.
        PathChainTask scoreTask = new PathChainTask(ScoreBasket, 0);
        tasks.add(scoreTask);

        tasks.add(new PathChainTask(finalparking, 0));
    }
